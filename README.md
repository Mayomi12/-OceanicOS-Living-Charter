...
The Charter is not only text — a live, zero-runtime OceanicOS is built under it in [`core/`](core/), one verified capability at a time (see [BUILD_LOG.md](BUILD_LOG.md)). Every capability ships with a browser verification suite; [`core/verify-all.html`](core/verify-all.html) is the single green light (**24/24 suites, 302 assertions**).
