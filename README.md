...
The Charter is not only text — a live, zero-runtime OceanicOS is built under it in [`core/`](core/), one verified capability at a time (see [BUILD_LOG.md](BUILD_LOG.md)). Every capability ships with a browser verification suite; [`core/verify-all.html`](core/verify-all.html) is the single green light (**21/21 suites, 266 assertions**).

The Vision Edition of the Agnostic Charter declares a **Root** — `minbpe` (Karpathy) — and a **Kernel**, the *Living Tokenization Studio*. That Root is now realized: [`core/studio.html`](core/studio.html) trains a byte-level BPE tokenizer, shows it learn, and reads it back — `decode(encode(text)) === text`, always. It offers two engines: the byte-level **Basic** tokenizer ([`core/tokenizer.js`](core/tokenizer.js), 0025) and a **GPT-style** one that splits on a regex before merging so tokens never cross word boundaries ([`core/regex-tokenizer.js`](core/regex-tokenizer.js), 0026) — with **special-token** boundary markers on top ([`core/special-tokens.js`](core/special-tokens.js), 0027), completing minbpe's Basic → Regex → special-tokens progression.
